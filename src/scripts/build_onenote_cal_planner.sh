...
YEAR_CAL_TEMP=Onenote-Planner-Yearly-Calendar
YEAR_PLAN_TEMP=Onenote-Planner-Yearly
QUAT_PLAN_TEMP=Onenote-Planner-Quatarly
WEEK_PLAN_TEMP=Onenote-Planner-Weekly
EMPTY_CAL_TEMP=Onenote-Planner-Empty
PLAN_TEMP_PATTERN=Planner-*.png
RES_DIR=../res
BUILD_DIR=../../build/build_on_planner
...
function build_quartarly()
{
    ...
    do
      ...
      echo "--- ...Q${i} | ${m1} | ${m2} | ${m3} | ${QUAT_PLAN_TEMP}"
...
function build_weekly()
{
    ...
    while [[ "$start_Mon" != "$end_Mon" ]]; do
      ...
      convert  \
        ...
        -fill black \
        -pointsize 14  \
        -draw "text 735, 75 '$(gcal -K --iso-week-number=yes -H no -s 1 ${month} ${year} |tail -n +2)'" \
        $src_file \
        $dest_file
...
function build_pdf()
{
  ...
  gm convert $BUILD_DIR/$PLAN_TEMP_PATTERN \
             -density 72 \
             -page a5 \
             $BUILD_DIR/onenote-planner-$year.pdf
  echo "-- ...done"
}
