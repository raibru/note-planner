...
WEEK_CAL_TEMP=RMPP-Planner-Weekly-Block-Col
...
function build_weekly_calendar()
{
    ...
    while [[ "$start_Mon" != "$end_Mon" ]]; do
      ...
      local dest_file=${BUILD_DIR}/Planner-${page_nr}-${ord}-Weekly_cal_${year}_${weeklead}_1.png
      ...
      convert  \
        ...
        -fill black \
        -pointsize 48 \
        -draw "text 50,82 '$header'" \
        -draw "text 710,82 '$kw'" \
        -draw "text 1400,82 '$year'" \
        -font helvetica \
        $src_file \
        $dest_file
